...
def draw_boxes(image, detected_boxes, labels, colors):
    ...
    for i, data in enumerate(detected_boxes):
        ...
        (pt1_x, pt1_y) = (int(data_box[i][0]), int(data_box[i][1]))
        (pt2_x, pt2_y) = (int(data_box[i][2]), int(data_box[i][3]))
...
def get_good_boxes(all_data, only_humans=False, min_precision=0.0, max_overlap=1.0):
    good_boxes = []
    actual_boxes = []
    for data in sorted(all_data, key=lambda x: x['score'], reverse=True):
        # print("x:{}, y:{}, w:{}, h:{}".format(x*size,y*size,w*size,h*size))
        if(data['score'] >= min_precision and (not only_humans or data['class']==0)): # evaluate precision
            if(len(actual_boxes)!=0):
                cb = get_box_coords(data['box'])
                ab_flag = False
